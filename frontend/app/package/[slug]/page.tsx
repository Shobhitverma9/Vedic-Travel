...
import ItineraryDownloadModal from "@/components/tours/ItineraryDownloadModal";
  ...
  useEffect(() => {
    const fetchTour = async () => {
      ...
      try {
        ...
        try {
          const settingsData = await settingsService.getSetting(
          ...
          if (
            ...
            settingsData.value.length > 0
          ) {
            const globalToursResponse = await toursService.getAllTours({
              ids: settingsData.value,
              limit: 12,
            });
            // Sort hydrated tours to match the order of IDs in the setting
            const hydrated = settingsData.value
              .map((id: string) =>
                globalToursResponse.tours.find((t: any) => t._id === id),
              )
              .filter((t: any) => t && t._id !== data._id);
            setOtherTours(hydrated.slice(0, 4));
          } else {
            // Fallback to existing logic if no global settings
            const allToursResponse = await toursService.getAllTours({
              limit: 5,
            });
            const filteredOtherTours = (allToursResponse.tours || [])
              .filter((t: any) => t._id !== data._id)
              .slice(0, 4);
          ...
          <div className="flex flex-wrap justify-between items-start gap-4">
            <div>
              ...
              <p className="text-lg text-gray-700 font-medium">
                {tour.nightDistribution ||
                  `2N ${tour.locations?.[0]} | 1N ${tour.locations?.[1]}`}
              </p>
            </div>
            <div className="flex flex-col items-end gap-2">
              {tour.itineraryPdf && (
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-white text-deepBlue border-2 border-deepBlue rounded-xl font-bold hover:bg-deepBlue hover:text-white transition-all transform hover:-translate-y-1 shadow-md group"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="mb-6">
              <TourPageHero
                images={
                  tour.slideshowImages?.length > 0
                    ? tour.slideshowImages
                    : tour.yatraImages?.thumbnailImages?.length > 0
                      ? tour.yatraImages.thumbnailImages
                      : tour.images
                }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 lg:gap-0 lg:divide-x divide-gray-200/80 items-stretch">
                ...
                <div className="flex items-center gap-3 px-4 py-2 group hover:bg-white transition-all rounded-xl lg:rounded-none">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-deepBlue text-sm whitespace-nowrap">
                      {nights}N / {tour.duration}D
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 px-4 py-2 group hover:bg-white transition-all rounded-xl lg:rounded-none">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-deepBlue text-sm leading-tight whitespace-normal break-words">
                      Ex: {joiningFrom}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-center px-4 py-2 group hover:bg-white transition-all rounded-xl lg:rounded-none">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    {(tour.packageIncludes && tour.packageIncludes.length > 0
                      ...
                      .map((item: string) => (
                        ...
                        >
                          ...
                          <span className="text-[8px] font-bold text-gray-400/80 uppercase tracking-tighter mt-0.5 group-hover/icon:text-deepBlue flex-shrink-0">
                            {item}
                          </span>
                        </div>
                      ))}
                    {tour.packageIncludes?.length > 5 && (
                      <div
                        className="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200"
                        title="More inclusions"
                      >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                      {(
                        ...
                        .split("/")
                        .map((place: string, idx: number, arr: any[]) => (
                          <div key={idx} className="flex items-center gap-3">
                            <span className="font-bold text-deepBlue text-[14px] tracking-tight break-words">
                              {place.trim()}
                            </span>
                            {idx < arr.length - 1 && (
                              <svg
              ...
              <div className="relative flex flex-col lg:flex-row items-center justify-between gap-8 text-center lg:text-left">
                <div className="space-y-1">
                  ...
                  <p className="text-white/70 text-sm font-medium">
                    Connect with our Yatra specialists for a customized
                    experience.
                  </p>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 md:p-8 mb-8">
              <div className="flex items-center justify-between px-8 mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-bold text-xl text-gray-400 tracking-wider">
                    HOTELS
                  </span>
                  <span className="text-sm font-medium text-gray-500 italic ml-2">
                    - Or similar 4 star hotel
                  </span>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 px-4">
                {tour.hotels?.map((hotel: any, index: number) => {
                  const hotelImages = hotel.images?.length > 0 ? hotel.images : (hotel.image ? [hotel.image] : []);
                  return (
                    <div
                      key={index}
                      className="flex flex-col bg-gray-50 rounded-2xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 group"
                    >
                      {/* Image Gallery */}
                      <div className="relative h-48 sm:h-56 bg-gray-200 overflow-hidden">
                        ...
                        {hotelImages.length > 1 && (
                          <>
                            <div className="absolute bottom-3 right-3 bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded-full backdrop-blur-sm shadow-sm z-10 flex items-center gap-1">
                              <Camera className="w-3 h-3" />
                              {hotelImages.length} Photos
                            </div>
                            <div className="absolute inset-x-0 bottom-0 h-1 bg-white/20 z-10">
                              <div className="h-full bg-saffron w-1/4 animate-pulse"></div>
                            </div>
                          </>
                        )}
                      </div>
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        <div className="flex justify-between items-start mb-2 gap-2">
                          <h4 className="font-bold text-lg text-deepBlue leading-tight group-hover:text-saffron transition-colors">
                            {hotel.name}
                          </h4>
                          <div className="flex text-yellow-400 text-sm flex-shrink-0 bg-white/50 px-2 py-0.5 rounded-full border border-yellow-100">
                        ...
                        <div className="mt-auto pt-3 border-t border-gray-200/50 flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            ...
            <div
              ref={inclusionsRef}
              id="inclusions-section"
              className="grid md:grid-cols-2 gap-6 mb-8"
            >
              <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-6 relative mt-4">
                <h3 className="absolute -top-4 left-6 bg-white px-2 font-handwriting text-xl text-green-600 font-bold">
                  Inclusions:
                </h3>
              ...
              <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-6 relative mt-4">
                <h3 className="absolute -top-4 left-6 bg-white px-2 font-handwriting text-xl text-red-600 font-bold">
                  Exclusions:
                </h3>
                <ul className="space-y-3 mt-2">
            ...
            <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-6 relative mb-8 mt-4">
              <h3 className="absolute -top-4 left-6 bg-white px-2 font-handwriting text-xl text-deepBlue">
                Things To Carry:
              </h3>
              <ul className="grid grid-cols-2 gap-2 mt-2">
                {(tour.thingsToCarry || []).map((item: string, i: number) => (
                  <li
                    key={i}
                    className="flex items-center gap-2 text-sm text-gray-700 break-words"
                  >
        ...
        <div className="bg-gray-50/50 py-16 border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
              <div>
                <h2 className="text-3xl md:text-4xl font-display font-bold text-deepBlue mb-2">
                  Others are also choosing..
                </h2>
                <p className="text-gray-600 font-medium italic">
                  Handpicked experiences from our most popular spiritual
                  journeys.
                </p>
              </div>
