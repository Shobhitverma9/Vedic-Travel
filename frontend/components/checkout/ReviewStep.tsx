...
export default function ReviewStep({
  ...
  const isPartialPayment =
    paidAmount && Math.abs(paidAmount - fullGrandTotal) > 1;
  ...
  const rows = [
    {
      icon: <MapPin size={15} className="text-saffron" />,
      label: "Starting From",
      value: departureCity,
    },
    {
      icon: <Tag size={15} className="text-saffron" />,
      label: "Tour Type",
      value: tourType,
    },
    {
      icon: <Package size={15} className="text-saffron" />,
      label: "Package Type",
      value: packageType,
    },
    {
      icon: <Calendar size={15} className="text-saffron" />,
      label: "Date of Travel",
      value: formattedDate,
    },
    {
      icon: <Users size={15} className="text-saffron" />,
      label: "Travellers",
      value: `Room 1 — ${adults} Adult${adults > 1 ? "s" : ""}`,
    },
  ];
            ...
            <div className="divide-y divide-gray-100">
              {rows.map((row, i) => (
                <div key={i} className="flex items-center justify-between py-3">
                  ...
                  <span className="font-medium text-[#1A2332] text-sm text-right">
                    {row.value}
                  </span>
                </div>
              ))}
          ...
          <div className="p-5 space-y-3 text-sm">
            ...
            <div className="border-t border-dashed border-gray-200 pt-3 flex flex-col gap-2">
              ...
              <div className="flex justify-between items-center bg-orange-50 -mx-5 px-5 py-3 mt-1">
                <span className="font-bold text-[#1A2332] uppercase tracking-wider text-xs">
                  Amount to Pay Now
                </span>
            ...
            <p className="text-[11px] text-gray-500 italic mt-2">
              Infant (0 to 3 yrs) don&apos;t occupy any seat
            </p>
          </div>
        </div>
